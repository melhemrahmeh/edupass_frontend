...
const reducer = (state, action) => {
  switch (action.type) {
    case "load-volunteer-start":
      return { ...state, loading: true };
    case "load-volunteer-success":
      return {
        ...state,
        loading: false,
        volunteerOpportunities: action.volunteerOpportunities,
      };
...
const Volunteer = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://edupass-rest-apis.herokuapp.com/api/opportunities/")
      ...
      .then((res) => {
        dispatch({
          type: "load-volunteer-success",
          volunteerOpportunities: res,
        });
      })
      .catch((err) => {
  ...
  const openVolunteerModal = useCallback((e, data) => {
    console.log(data);
    dispatch({
      type: "add-volunteer-modal-is-open",
      id: data.volunteerId,
